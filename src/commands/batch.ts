...
import type { BatchCheckResponse } from "../types.js";
import {
  isPlain,
  bold,
  green,
...
export function registerBatchCommand(program: Command): void {
  program
    ...
    .option(
      "-m, --mode <mode>",
      "Check mode: quick or full",
      "quick",
    )
    .action(async (rawNames: string[], opts, cmd) => {
      const globals = cmd.optsWithGlobals();

      if (rawNames.length < 2 || rawNames.length > 50) {
        console.error(
          "Error: Provide 2-50 brand names for batch check.",
        );
        process.exit(1);
      }
      ...
      const spinner =
        !globals.json && !isPlain()
          ? ora(
              `Batch checking ${names.length} names (${opts.mode})...`,
            ).start()
          : null;
      ...
      try {
        ...
        console.log(
          bold(
            `Batch Results (${data.count} names, ${data.mode} mode)`,
          ),
        );
        console.log("");
        ...
        for (const r of data.results) {
          ...
          const score = color(
            `${r.score.score}/${r.score.maxScore}`.padEnd(8),
          );
