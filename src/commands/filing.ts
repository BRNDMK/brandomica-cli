...
import type { FilingReadinessResponse } from "../types.js";
import {
  isPlain,
  bold,
  green,
  ...
  program
    ...
    .action(async (rawName: string, opts, cmd) => {
      ...
      try {
        ...
        console.log(
          `  ${verdictBadge(fr.verdict)}  Risk: ${fr.filingRisk}  Confidence: ${fr.confidence}`,
        );

        if (
          fr.missingCriticalCategories.length > 0
        ) {
          console.log(
            `  ${yellow("Missing:")} ${fr.missingCriticalCategories.join(", ")}`,
          );
        }
        ...
        if (fr.topConflicts.length > 0) {
          ...
          for (const c of fr.topConflicts) {
            ...
            const jurisdiction = c.jurisdiction
              ...
              : "";
            console.log(
              `    ${sev} ${c.source}${jurisdiction}: ${c.description}`,
            );
