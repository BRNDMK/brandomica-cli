...
import {
  formatDomainsTable,
  formatSocialTable,
  formatTrademarksTable,
  formatAppStoresTable,
  formatSaasTable,
  formatGoogleTable,
} from "./table.js";
import { formatScoreCard, formatSafetyCard } from "./score.js";

export function formatReport(report: BrandReport): string {
  const lines: string[] = [];
  ...
  lines.push(
    `  Verdict: ${fr.verdict.toUpperCase()} (risk: ${fr.filingRisk})`,
  );
  ...
  if (fr.topConflicts.length > 0) {
    ...
    for (const c of fr.topConflicts) {
      lines.push(
        `    ${dim("\u2192")} [${c.severity}] ${c.source}: ${c.description}`,
      );
    }
  }
  ...
  lines.push(
    `  USPTO (TEAS Plus): ~$${report.trademarkFilingEstimates.uspto.perClass}/class`,
  );
  lines.push(
    `    ${dim(report.trademarkFilingEstimates.uspto.note)}`,
  );
  lines.push(
    `  EUIPO: ~EUR ${report.trademarkFilingEstimates.euipo.perClass}/class`,
  );
  lines.push(
    `    ${dim(report.trademarkFilingEstimates.euipo.note)}`,
  );
  lines.push("");
  ...
  lines.push("");
  lines.push(
    dim(`Checks completed: ${report.checksCompleted}/${report.checksTotal}`),
  );
