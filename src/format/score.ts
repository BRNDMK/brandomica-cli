import type { ScoreResult, BrandSafetyAssessment } from "../types.js";
import {
  bold,
  dim,
  green,
...
export function formatScoreCard(score: ScoreResult): string {
  const color = scoreColor(score.score, score.maxScore);
  const lines: string[] = [];
  lines.push(bold("Availability Score"));
  lines.push(
    `  ${color(`${score.score}/${score.maxScore}`)} ${dim(`(${score.confidence} confidence, ${score.totalChecks} checks)`)}`,
  );
  if (
    ...
    lines.push(
      `  ${SYM.warn} ${yellow("Low confidence")} \u2014 missing: ${score.missingCriticalCategories.join(", ")}`,
    );
  }
...
export function formatSafetyCard(
  safety: BrandSafetyAssessment,
): string {
  const lines: string[] = [];
  lines.push(bold("Brand Safety Assessment"));
  lines.push(
    `  ${riskBadge(safety.overallRisk)}  ${bold(`${safety.safetyScore}/100`)} \u2014 ${safety.headline}`,
  );
  ...
  for (const signal of safety.signals) {
    ...
    lines.push(
      `  ${color(riskLabel)} ${signal.label}: ${signal.summary}${criticalMark}`,
    );
  }
