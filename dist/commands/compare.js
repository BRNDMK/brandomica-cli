...
function formatCompareRow(r) {
    const lines = [];
    const color = scoreColor(r.score.score, r.score.maxScore);
    lines.push(`  ${bold(r.name.padEnd(20))} Score: ${color(`${r.score.score}/${r.score.maxScore}`)}  Safety: ${riskBadge(r.safety.overallRisk)} ${r.safety.safetyScore}/100`);
    const available = r.domains.filter((d) => d.available).length;
    const total = r.domains.length;
    lines.push(`    Domains: ${available}/${total} available  .com: ${r.domains.find((d) => d.domain.endsWith(".com"))?.available ? green("yes") : red("no")}`);
    const socialAvail = r.social.filter((s) => s.available === true).length;
    const socialTaken = r.social.filter((s) => s.available === false).length;
    lines.push(`    Social: ${green(`${socialAvail} free`)} ${red(`${socialTaken} taken`)}`);
    const tmConflicts = r.trademarks
        .filter((t) => t.available === false)
        .reduce((sum, t) => sum + t.count, 0);
    lines.push(`    Trademarks: ${tmConflicts === 0 ? green("clear") : red(`${tmConflicts} conflicts`)}`);
    if (r.safety.blockers.length > 0) {
        lines.push(`    ${red("Blockers:")} ${r.safety.blockers.join("; ")}`);
    }
    return lines.join("\n");
}
export function registerCompareCommand(program) {
    program
        ...
        .action(async (rawNames, _opts, cmd) => {
        ...
        try {
            ...
            if (data.recommendation) {
                ...
                console.log(`${bold("Recommendation:")} ${data.recommendation}`);
            }
        }
        catch (err) {
