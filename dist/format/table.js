import { green, red, yellow, dim, bold, availabilitySymbol, SYM, } from "./colors.js";
function padEnd(s, len) {
    const stripped = s.replace(/\x1B\[[0-9;]*m/g, "");
    const pad = Math.max(0, len - stripped.length);
    return s + " ".repeat(pad);
}
export function formatDomainsTable(domains) {
    const lines = [bold("Domains")];
    for (const d of domains) {
        ...
        if (d.available) {
            ...
            if (d.purchasePrice != null && d.renewalPrice != null) {
                ...
                price = dim(`  $${d.purchasePrice}/yr1  $${d.renewalPrice}/renew  $${tco3}/3Y`);
                if (minYears > 1) {
...
export function formatSocialTable(social) {
    const lines = [bold("Social Handles")];
    for (const s of social) {
        const sym = availabilitySymbol(s.available);
        const name = padEnd(s.platform, 16);
        const status = s.available === true
            ? green("available")
            : s.available === false
                ? red("taken")
...
export function formatTrademarksTable(trademarks) {
    const lines = [bold("Trademarks")];
    for (const t of trademarks) {
        const sym = availabilitySymbol(t.available);
        const name = padEnd(t.source, 12);
        const status = t.available === true
            ? green("clear")
            : t.available === false
...
export function formatAppStoresTable(appStores) {
    const lines = [bold("App Stores")];
    for (const a of appStores) {
        const sym = a.found === true ? SYM.fail : a.found === false ? SYM.ok : SYM.warn;
        const name = padEnd(a.platform, 16);
        const status = a.found === true
...
export function formatSaasTable(saas) {
    const lines = [bold("Package Registries & SaaS")];
    for (const s of saas) {
        const sym = availabilitySymbol(s.available);
        const name = padEnd(s.platform, 16);
        const status = s.available === true
...
export function formatGoogleTable(google) {
    const lines = [bold("Web Presence (Google)")];
    for (const g of google) {
        const sym = g.found === true ? SYM.fail : g.found === false ? SYM.ok : SYM.warn;
        const name = padEnd(g.platform, 16);
        const status = g.found === true
            ? red(`competitors found${g.resultCount ? ` (${g.resultCount})` : ""}`)
            : g.found === false
